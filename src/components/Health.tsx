...
export function Health({ query, report, archives, onSelect, onRestore }: {
  ...
  const findings = [...report.findings]
    .filter((finding) => `${t(finding.title.key, finding.title.params)} ${t(finding.detail.key, finding.detail.params)} ${report.skills.find((skill) => skill.id === finding.skillId)?.name ?? ''}`.toLowerCase().includes(query.toLowerCase()))
