...
export function Inspector({ skill, findings, canLocalize, uninstallInstallations, onLocalize, onUninstall, onEdit, onReveal, onCopyHandoff, onTrust, onCheckReputation }: {
  ...
  return <aside className="inspector">
    ...
    <InspectorSection title={t('inspector.localFindings')}>{findings.length ? <ul className="finding-list">{findings.map((finding) => <li key={finding.id}><span className={`severity ${finding.severity}`} /><div><strong>{t(finding.title.key, finding.title.params)}</strong><small>{t(finding.detail.key, finding.detail.params)}</small></div></li>)}</ul> : <p className="healthy-copy">{t('health.noFindingsExactCopy')}</p>}</InspectorSection>
  </aside>
}
