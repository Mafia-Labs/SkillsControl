...
export function Overview({ report, onViewHealth, onViewMap }: { report: ScanReport, onViewHealth: () => void, onViewMap: () => void }) {
  ...
  return <>
    <section className="hero">
      <div><p className="eyebrow">System diagnosis</p><h2>Your agent skills, made legible.</h2><p>Inventory every local installation, find the risky ones, and deploy intentional configurations without hand-editing folders.</p></div>
    ...
    <section className="stat-grid" aria-label="Inventory summary">
      ...
      <Stat value={countUniqueProjects(report)} label="Projects" detail="With discovered skills" />
      <Stat value={countDuplicates(report)} label="Duplicates" detail="Worth reviewing" />
    </section>
    <section className="two-column">
      <div className="panel diagnosis"><PanelHeading title="Recommended next actions" action="View all" onAction={onViewHealth} />
        {errors + warnings === 0 ? <Empty icon="✓" title="Everything looks healthy" detail="No structural issues were found in the scanned skills." /> : <>
          <ActionItem label={`${errors} urgent issue${errors === 1 ? '' : 's'}`} detail="Fix invalid or outdated skills first" tone="error" onClick={onViewHealth} />
          <ActionItem label={`${warnings} review suggested`} detail="Inspect scripts and local modifications" tone="warning" onClick={onViewHealth} />
          <ActionItem label="Check scope collisions" detail={`${countDuplicates(report)} skills appear in more than one location`} tone="info" onClick={onViewMap} />
        </>}
      </div>
      <div className="panel footprint"><PanelHeading title="Context footprint" />
        ...
        <p>Estimate based on the scanned <code>SKILL.md</code> files. It indicates potential load, not actual agent token usage.</p>
